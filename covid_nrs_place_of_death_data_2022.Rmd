...
weekly_cause_of_death_2021_onwards <-
  map_dfr(
...
The reports available from NRS in 2022 include data from 2021-2022, but we can also include data from 2020 for context. For this I'll load the last 'main report' file from 2021.
...
weekly_deaths_cause_of_death_2020 <-
  readxl::read_excel(files_historical_deaths$`2020-2021`, sheet = nrs_weekly_data_sheet_2020, col_names = headers_in_nrs_data_2020, range = cell_rows(6:125)) %>%  # the data are inrows 6:125
...
#### Merge 2020 data with current data
...
merged_deaths_cause_of_death <-
  ...
  mutate(historic_period = historic_period_depending_on_year(year)) %>%  # add historic period so we match with the correct historic period data
  inner_join(
    historical_average_cause_of_death, by = c("place_of_death", "week_number", "cause_of_death", "historic_period", "year", "date_w_c")
  )
```
...
```{r}
# TODO: work out what to do about this - is it even used?


## bespoke computation because format is different... deaths split by cause!
## Note: this isn't appropriate for 2022 yet because we don't have the annual death figure yet
