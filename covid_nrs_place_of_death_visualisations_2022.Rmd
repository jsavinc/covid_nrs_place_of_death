...
# TODO: make the plot nicer, and print out the predicted year somewhere
# TODO: figure out how to adjust the incomplete 2022 data
actual_data <- tibble(year = c(2020:2022)) %>% 
  left_join(
    merged_proportions_of_deaths_by_place %>%
  ...
  mutate(
    predicted_year =  if_else(
      condition = year != 2022,
      true = 2015 + (number_of_deaths - coef(regression_model)[1]) / coef(regression_model)[2],
      ## for 2022, we use slightly different computation
      false = NA_real_
    ),
    label = paste0("Actual home deaths in ", year)
  )
  

ggplot(data = predicted_data, aes(x=year, y=predicted)) + 
  geom_point(data = regression_data, aes(x = year, y = number_of_deaths)) +
  geom_line(linetype = "dashed") +
  ggrepel::geom_label_repel(data = actual_data, aes(x = predicted_year, y = number_of_deaths, label = label)) +
  geom_vline(data = actual_data, aes(xintercept = predicted_year)) +
  xlim(c(2015,2035)) +
  theme_minimal() +
