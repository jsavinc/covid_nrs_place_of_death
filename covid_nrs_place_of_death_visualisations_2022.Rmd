...
## combine maps
(figure_map_home_deaths_increase_vs_covid_case_from_2020_la <-
  wrap_plots(
    figure_map_home_deaths_increase_from_2020_la +
...
(figure_map_covid_cases_by_year_la <-
  weekly_covid_cases_la %>%
  ...
  distinct %>%
  left_join(shapefile_la, by=c("ref_area"="LAD20NM")) %>%
  # ggplot(aes(geometry=geometry, fill=log(ratio_covid_cases)), colour="grey10") +
  ggplot(aes(geometry=geometry, fill=ratio_covid_cases), colour="grey10") +
  geom_sf(colour="grey10") +
  # scale_fill_viridis_b(option = "B", labels = function(x) format(exp(x), big.mark=",")) +
  scale_fill_viridis_b(option = "B", labels = function(x) format(x, big.mark=",")) +
  theme(axis.text = element_blank(), panel.grid = element_blank()) +
  facet_wrap(~year, nrow = 1) +
  labs(x="", y="",
