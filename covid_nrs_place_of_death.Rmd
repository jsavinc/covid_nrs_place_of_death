...
## Figure 7: deaths at home, all of Scotland
...
walk(
  .x = c(".pdf",".svg",".png"),
  .f = ~ggsave(filename = paste0("./outputs/figure_7_deaths_at_home",.x), plot = figure_7_deaths_at_home, width = 12, height = 9, dpi = 300)
)
...
```{r}
# TODO: check the sex-disaggregated data, the historical figures look wrong
merged_data_sex %>%
  # filter(sex == "all" & age == "all") %>%
  filter(place_of_death %in% c("Home & other non-institution")) %>%
  pivot_longer(cols = c(deaths_all_causes, deaths_non_covid), 
