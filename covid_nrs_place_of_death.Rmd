...
weekly_deaths_sex_age <-  # structure here is a bit more complicated - headers in row 3, but sex & age headers in row 4
  readxl::read_excel(weekly_deaths_sex_age_file, sheet = 2, skip = 4, col_names = c("week_number","place_of_death","sex","age","cause_of_death","number_of_deaths"), guess_max = 1e5) %>%
...
past_deaths_hb <- 
  readxl::read_excel(past_deaths_hb_file, sheet = 2, skip = 2, col_names = TRUE) %>%  # first two rows are empty, row 3 has headers
  clean_names() %>%
  drop_na(number_of_deaths) %>%  # remove empty rows & one row where it's just the copyright notice
...
past_deaths_la <-
  readxl::read_excel(past_deaths_la_file, sheet = 2, skip = 2, col_names = TRUE) %>%  # first two rows are empty, row 3 has headers
  clean_names() %>%
  drop_na(number_of_deaths) %>%  # remove empty rows & one row where it's just the copyright notice
  rename(  # same format as current data
    week_number = matches("week"),
...
past_deaths_sex_age <-  # structure here is a bit more complicated - headers in row 3, but sex & age headers in row 4
  readxl::read_excel(past_deaths_sex_age_file, sheet = 2, skip = 4, col_names = c("year","week_number","place_of_death","sex","age","number_of_deaths"), guess_max = 1e5) %>%
  drop_na(number_of_deaths) %>%    # remove empty rows & one row where it's just the copyright notice
  mutate(
...
TODO: figure out which boundaries in Scotland correspond to council areas!

~~To get council areas in Scotland we need NUTS 2 level map data~~, and the most appropriate level of detail to use is likely the BUC - Ultra Generalised Clipped
...
https://geoportal.statistics.gov.uk/datasets/nuts-level-2-january-2018-ultra-generalised-clipped-boundaries-in-the-united-kingdom?geometry=-12.122%2C55.353%2C4.566%2C57.479

health board boundaries April 2019, BUC
...
getting error trying to use shapefile with ggplot:
> Error in CPL_transform(x, crs, aoi, pipeline, reverse) : 
  crs not found: is it missing?

explicitly setting crs on shapefile fails,

version fo sf 0.9-7:
> Linking to GEOS 3.8.0, GDAL 3.0.4, PROJ 6.3.1

```{r}
# TODO: learn how to use maps etc.
zip_file <- tempfile()
unzipped <- tempfile()

## Scottish LAU 2011 level 1
# curl_download(url = "https://borders.ukdataservice.ac.uk/ukborders/easy_download/prebuilt/shape/Scotland_laulevel1_2011.zip", destfile = zip_file, quiet = FALSE)  # download file from ONS
# name <- list.files(unzipped, pattern=".shp")
# shapefile <- st_read(file.path(unzipped, name))

## below fails, saying cannot allocate vector of size XX MB/GB
# curl_download(url = "https://opendata.arcgis.com/datasets/7f83b82ef6ce46d3a5635d371e8a3e7c_0.zip?outSR=%7B%22latestWkid%22%3A27700%2C%22wkid%22%3A27700%7D", destfile = zip_file, quiet = FALSE)  # download file from ONS

download_and_import_shapefile <- function(url) {
  zip_file <- tempfile()
  unzipped <- tempfile()
  downloaded_zip <- curl_download(url = url, destfile = zip_file, quiet = FALSE) 
  unzip(zipfile = zip_file, exdir = unzipped)
  name <- list.files(unzipped, pattern=".shp$")
  shapefile <- st_read(file.path(unzipped, name))
  return(shapefile)
}

health_boards_shapefile_url <- "https://maps.gov.scot/ATOM/shapefiles/SG_NHS_HealthBoards_2019.zip"
la_shapefile_url <- "https://opendata.arcgis.com/datasets/3a4fa2ce68f642e399b4de07643eeed3_0.zip?outSR=%7B%22latestWkid%22%3A27700%2C%22wkid%22%3A27700%7D"


shapefile_hb <- download_and_import_shapefile(url = health_boards_shapefile_url)
shapefile_la <- download_and_import_shapefile(url = la_shapefile_url)
...
# shapefile_la %>%
#   inner_join(merged_deaths_la, by = c("lad19nm"="ref_area"))

# shapefile_hb %>%
#   inner_join(merged_deaths_hb, by = c("HBName"="ref_area"))

# TODO: summarise data by LA/HB so it can be shown on a map
# otherwise it loads forever because we're re-plotting the same regional boundary for each row!!
...
# ggplot(data = shapefile_hb, aes(geometry=geometry)) + geom_sf()
  
# merged_deaths_hb %>%
#   left_join(shapefile_hb,by=c("ref_area"="HBName")) %>%
#   ggplot(aes(fill = deaths_mean, geometry = geometry)) +
#   geom_sf()
...
```{r, fig.width=12, fig.height=9, warning=FALSE}
removed_other_category_text <- glue::glue("Excluding 'Other' institutional place of death, N={merged_data_overall %>% filter(sex == 'all' & age == 'all' & place_of_death == 'Other') %>% .$deaths_all_causes %>% sum} for weeks 1-{max(merged_data_overall %>% filter(year==2020) %>% .$week_number)} in 2020 and 1-{max(merged_data_overall %>% filter(year==2021) %>% .$week_number)} in 2021.")

annotations_figure_place_of_death <-
# datapasta::tribble_paste(merged_data_overall %>% select(place_of_death))
  tibble::tribble(
  ...
  mutate(
    place_of_death = factor(place_of_death, levels = levels(merged_data_overall$place_of_death)),  # needs to be factor with same levels, otherwise it messes up level order when added as geom_text later!
...
```{r, fig.width=12, fig.height=9, warning=FALSE}
removed_under_15_text <- glue::glue("Excluding individuals aged <15 at death, total N={merged_data_age %>% filter(age %in% c('0','1-14') & place_of_death == 'Home & other non-institution') %>% .$deaths_all_causes %>% sum} for weeks 1-{max(merged_data_age %>% filter(year==2020) %>% .$week_number)} in 2020 and 1-{max(merged_data_age %>% filter(year==2021) %>% .$week_number)} in 2021.")
