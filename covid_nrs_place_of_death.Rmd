...
# beds_raw <- 
#   read_csv("./beds_by_nhs_board_of_treatment_and_specialty.csv", guess_max = 3e4) %>%
...
NOTE: I've commented this out - I ended up not using it. TODO: remove in a future iteration!
...
(figure_annual_deaths_by_cause <-
  ...
  # pivot_longer(cols = c(deaths, annual_deaths_mean), names_to = "deaths_type", values_to = "deaths") %>%
  ggplot() +
  # geom_col(aes(x = cause_of_death, fill = cause_of_death, y = deaths, group = deaths_type, linetype = deaths_type), position = "dodge", colour = "grey10") +
  geom_col(aes(x = cause_of_death, fill = cause_of_death, y = deaths), colour = "grey10") +
  # geom_point(aes(x = cause_of_death, y = annual_deaths_mean), colour = "grey10") +
  geom_line(aes(x = cause_of_death, y = annual_deaths_mean), colour = "grey10") +
...
(figure_deaths_at_home_by_cause <-
  ...
  # annotate(geom = "rect", xmin = compute_start_date_from_week_number(week_number = 1, year_number = 2021), xmax = compute_start_date_from_week_number(week_number = weeks_available_2021, year_number = 2021), ymin = -Inf, ymax = Inf, fill = "grey90", alpha = 0.2) +  # shade area - geom_rect doesn't work here
  scale_x_date(date_labels = "%d-%m-%y", date_breaks = "8 weeks", limits = c(earliest_date, most_recent_date_available_2021)) +
  geom_line(aes(y = number_of_deaths, colour = cause_of_death, linetype = deaths_type)) +
  scale_colour_viridis_d() +
  scale_linetype_manual(labels = c("Historic mean 2015-2019","Deaths since 2020"), values = c("dashed","solid")) +
  # geom_line(aes(y = number_of_deaths  , colour = deaths_type)) +
  # geom_line(aes(y = deaths_mean), linetype = "dashed", colour="grey20") +
  facet_wrap(cause_of_death~place_of_death, scales = "free_y") +
  theme(
    legend.position = "top",
    legend.title = element_blank(),
    plot.caption = element_text(size = 10, colour = "gray60"),
  ...
  labs(x = "Date w/c",
       y = "Weekly Deaths (N)",
       subtitle = date_range_text,
       caption = paste0(c("Note that y-axis scale varies across panels!",death_registrations_text,removed_other_category_text,source_nrs_text), collapse="\n")
       ) +
  NULL
)
