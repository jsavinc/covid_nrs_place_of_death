...
# TODO: show that only 2015 had week 53 and deal with this somehow - combine next years week 1 and previous week 52 instead?
week_53_text <- "Due to the way ISO 8601 counts weeks, only 2015 in the 2015-2019 period had 53 weeks."
```
...
```{r}
## check whether there's an issue with week 52/53
# 

merged_data <-
  data_main %>%
  left_join(tbl_recoded_place_of_death) %>%
  left_join(past_deaths %>% mutate(sex = "all", age = "all", cause_of_death = "all-causes"),  # only merge for aggregated data b/c that's all we have!
