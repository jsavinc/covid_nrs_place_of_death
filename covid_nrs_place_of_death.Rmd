...
This analysis is based off two sets of data:

1. Weekly deaths involving COVID-19 from Scottish gov't open data service
  - this is imported using the `opendatascot` package in R, and connects to the data available at: [the Scottish Gov't open data   website](https://statistics.gov.scot/data/deaths-involving-coronavirus-covid-19)
2. Historical deaths 2015-2019, grouped by week & location
  - These are available at the NRS website on [deaths involving COVID-19]( https://www.nrscotland.gov.uk/statistics-and-data/statistics/statistics-by-theme/vital-events/general-publications/weekly-and-monthly-data-on-births-and-deaths/deaths-involving-coronavirus-covid-19-in-scotland), specifically under the 'Related Statistics' section, under the heading 'Ad-hoc queries'.
  - We load the total deaths, but also grouped by:
    - Health board
    - Local Authority
    - Sex
    - Age
...
Note that the weekly deaths due to COVID-19 also include a weekly average 2015-2019, but for the purposes of showing the range of deaths (weekly minima & maxima), we need the other datasets. The same holds for the age & sex breakdown.
...
deaths_by_cause %>% count(ref_period, date_w_c) %>% slice(1:3, (nrow(.) - 3):nrow(.)) %>% knitr::kable(caption = "Checking the computation of 'date w/c' from year & week number - note how the 1st week of 2020 started in 2019.")
```
...
unique_places_of_death_from_data <- map(
  .x = list(deaths_by_cause, past_deaths_overall, past_deaths_health_board, past_deaths_council_area, past_deaths_age_sex),
...
merged_deaths_by_health_board <-
  deaths_by_cause %>%
  split_deaths_due_to_covid_and_not %>%
  left_join(historical_range_by_health_board) %>%
  compute_run_over_week_for_2021
...
removed_other_category_text <- glue::glue("Excluding 'Other' institutional place of death, N={merged_data %>% filter(sex == 'all' & age == 'all' & place_of_death_recoded == 'Other') %>% .$deaths_all_causes %>% sum} for weeks 1-{max(merged_data %>% filter(year==2020) %>% .$week_number)} in 2020 and 1-{max(merged_data %>% filter(year==2021) %>% .$week_number)} in 2021.")
