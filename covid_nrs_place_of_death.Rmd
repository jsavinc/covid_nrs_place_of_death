...
past_deaths_hb <- 
  ...
  tidyr::complete(
    ...
    health_board, place_of_death,  # fill in missing combinations of hb & pod with 0 deaths
    fill = list(number_of_deaths = 0)
  ) %>% 
...
past_deaths_la <-
  ...
  tidyr::complete(
    nesting(week_number,year),  # unique combinations that we don't want to combinatorily explode
    council_area, place_of_death,  # fill in missing combinations of la & pod with 0 deaths
    fill = list(number_of_deaths = 0)
  ) %>% 
  recode_place_of_death()  # recode place of death after completing missing combinations, otherwise the factor level "All" gets exploded also
...
To get council areas in Scotland we need NUTS 2 level map data.q
...
# merged_deaths_la %>%
#   left_join(shapefile,by=c("ref_area"="name")) %>%
...
The average will be computed to the "left" of the current value (a week's moving average is the average of values at this week + n-1 past weeks)

```{r}
weeks_to_average <- 2


# TODO: implement rolling averages also - I think this needs to be applied at source tbls first (i.e. max of sum over two weeks rather than mean of maxes?)
  # group_by(ref_area, sex, age, place_of_death) %>% 
  # mutate(across(.cols = matches("deaths_"), .fns = ~zoo::rollmean(x = .x, k = 2, align = "right", fill = NA)))
...
historical_average_age <-
  ...
  summarise(number_of_deaths = sum(number_of_deaths), .groups = "drop") %>%  # recompute the sum of deaths over sexes
  group_by(
...
historical_average_sex <-
  ...
  create_week_53_data_for_years_with_52_weeks() %>%
  group_by_all %>%
  ungroup(age, number_of_deaths) %>%  # this is done by grouping over everything except the number of deaths & age
  summarise(number_of_deaths = sum(number_of_deaths), .groups = "drop") %>%  # recompute the sum of deaths over ages
  group_by(
