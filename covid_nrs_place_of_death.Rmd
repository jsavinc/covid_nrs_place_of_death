...
```{r}
## this is fairly ad-hoc... but it works!
table_comparing_rates <-
  bind_rows(
    ## all available data
    merged_deaths_overall %>%
      filter(age == "all" &
               sex == "all" &
               str_starts(place_of_death, "Home")) %>% group_by(year, week_number) %>%
      summarise(
        ...
        past_deaths_overall %>% filter(str_starts(place_of_death, "Home") &
                                         year != "average") %>% group_by(week_number) %>% summarise(
        ...
        deaths_non_covid = sum(deaths_non_covid, na.rm = TRUE),
        deaths_covid_related = sum(deaths_covid_related, na.rm = TRUE),
        past_annual_deaths = sum(average_weekly_deaths)
      )
    ,
    ## 2020 only
    bind_cols(
      merged_deaths_overall %>%
        filter(
          age == "all" &
            sex == "all" & str_starts(place_of_death, "Home") & year == 2020
        ) %>%
        summarise(
          period = "2020",
          deaths_all_causes = sum(deaths_all_causes),
      ...
      past_deaths_overall %>% filter(str_starts(place_of_death, "Home") &
                                       year != "average") %>% summarise(past_annual_deaths = sum(number_of_deaths) / 5)
    ),
    ...
    bind_cols(
      merged_deaths_overall %>%
        filter(
          age == "all" &
            sex == "all" &
            str_starts(place_of_death, "Home") &
            year == 2020 & week_number >= 12
        ) %>%
        summarise(
          period = "2020, from wk12",
          deaths_all_causes = sum(deaths_all_causes),
          deaths_non_covid = sum(deaths_non_covid, na.rm = TRUE),
          deaths_covid_related = sum(deaths_covid_related, na.rm = TRUE)
        ),
      past_deaths_overall %>% filter(
        week_number >= 12 &
          str_starts(place_of_death, "Home") &
          year != "average"
      ) %>% summarise(past_annual_deaths = sum(number_of_deaths) / 5)
    )
  ) %>%
  mutate(
    ratio_deaths_all_causes_to_historical = deaths_all_causes / past_annual_deaths,
    ratio_non_covid_deaths_to_historical = deaths_non_covid / past_annual_deaths,
    proportion_covid_deaths = deaths_covid_related / deaths_all_causes
  )

write_csv(x = table_comparing_rates, file = file.path(dir_outputs, "annual_death_rates_comparison_with_historical_deaths.csv"))

table_comparing_rates %>% knitr::kable(caption = "Comparing annual home death rate during COVID pandemic to historical deaths at home")
```
...
only_2021_text <- glue::glue("Data for 2021 only include weeks 1-{format(max(merged_deaths_age$week_number[merged_deaths_age$year==2021]))} (w/c {format(max(merged_deaths_age$date_w_c))}).")
...
walk(
  .x = c(".pdf",".svg",".png"),
  .f = ~ggsave(filename = paste0(dir_outputs,"/figure_proportion_home_deaths_over_time",.x), plot = figure_proportion_home_deaths_over_time, width = 12, height = 10, units = "cm", dpi = 300)
)
...
walk(
  .x = c(".pdf",".svg",".png"),
  .f = ~ggsave(filename = paste0(dir_outputs,"/figure_annual_deaths_by_age_group_2020",.x), plot = figure_annual_deaths_by_age_group_2020, width = 12, height = 7, dpi = 300)
)
...
walk(
  .x = c(".pdf",".svg",".png"),
  .f = ~ggsave(filename = paste0(dir_outputs,"/figure_place_of_death",.x), plot = figure_place_of_death, width = 12, height = 9, dpi = 300)
)
...
## All of Scotland, deaths at home
...
walk(
  .x = c(".pdf",".svg",".png"),
  .f = ~ggsave(filename = paste0(dir_outputs,"/figure_deaths_at_home",.x), plot = figure_deaths_at_home, width = 14, height = 11, units = "cm", dpi = 300)
)
...
walk(
  .x = c(".pdf",".svg",".png"),
  .f = ~ggsave(filename = paste0(dir_outputs,"/figure_deaths_at_home_2020",.x), plot = figure_deaths_at_home_2020, width = 12, height = 9, dpi = 300)
)
...
walk(
  .x = c(".pdf",".svg",".png"),
  .f = ~ggsave(filename = paste0(dir_outputs,"/figure_deaths_at_home_by_health_board",.x), plot = figure_deaths_at_home_by_health_board, width = 12, height = 9, dpi = 300)
)
...
walk(
  .x = c(".pdf",".svg",".png"),
  .f = ~ggsave(filename = paste0(dir_outputs,"/figure_deaths_at_home_2week_average_by_health_board",.x), plot = figure_deaths_at_home_2week_average_by_health_board, width = 12, height = 9, dpi = 300)
)
...
walk(
  .x = c(".pdf",".svg",".png"),
  .f = ~ggsave(filename = paste0(dir_outputs,"/figure_deaths_at_home_by_local_authority",.x), plot = figure_deaths_at_home_by_local_authority, width = 12, height = 9, dpi = 300)
)
...
walk(
  .x = c(".pdf",".svg",".png"),
  .f = ~ggsave(filename = paste0(dir_outputs,"/figure_deaths_at_home_2week_average_by_local_authority",.x), plot = figure_deaths_at_home_2week_average_by_local_authority, width = 12, height = 9, dpi = 300)
)
...
walk(
  .x = c(".pdf",".svg",".png"),
  .f = ~ggsave(filename = paste0(dir_outputs,"/figure_deaths_at_home_by_age",.x), plot = figure_deaths_at_home_by_age, width = 12, height = 9, dpi = 300)
)
...
walk(
  .x = c(".pdf",".svg",".png"),
  .f = ~ggsave(filename = paste0(dir_outputs,"/figure_deaths_at_home_by_sex",.x), plot = figure_deaths_at_home_by_sex, width = 12, height = 9, dpi = 300)
)
...
walk(
  .x = c(".pdf",".svg",".png"),
  .f = ~ggsave(filename = paste0(dir_outputs,"/figure_weekly_deaths_to_cases",.x), plot = figure_weekly_deaths_to_cases, width = 15, height = 15, units = "cm", dpi = 300)
)
...
home_deaths_ratio_text_text <- "Ratio computed from total home deaths in 2020 divided by mean annual home deaths 2015-2019"
...
figure_map_ratio_home_deaths_2020_to_historical_la <-
  ...
  labs(x="", y="",
       title = str_wrap("Ratio of home deaths in 2020 to historical mean 2015-2019, by Local Authority", width = 50),
...
walk(
  .x = c(".pdf",".svg",".png"),
  .f = ~ggsave(filename = paste0(dir_outputs,"/figure_map_ratio_home_deaths_2020_to_historical_la",.x), plot = figure_map_ratio_home_deaths_2020_to_historical_la, width = 9, height = 12, dpi = 300)
)
...
figure_map_ratio_home_deaths_2020_to_historical_hb <-
  ...
  labs(x="", y="",
       title = str_wrap("Ratio of home deaths in 2020 to historical mean 2015-2019, by Health Board", width=50),
       subtitle = str_wrap("A ratio of 1 means there was no increase; A ratio of 1.6 means there was a 60% increase of home deaths in 2020 over the mean 2015-2019.", width = 80),
       caption = paste0(c(areas_relatively_higher_number_home_deaths_text, home_deaths_ratio_text, source_nrs_text), collapse="\n"),
       fill = "Ratio"
       ) +
...
walk(
  .x = c(".pdf",".svg",".png"),
  .f = ~ggsave(filename = paste0(dir_outputs,"/figure_map_ratio_home_deaths_2020_to_historical_hb",.x), plot = figure_map_ratio_home_deaths_2020_to_historical_hb, width = 9, height = 12, dpi = 300)
)
...
figure_map_per_capita_covid_cases_2020_la <-
  ...
  theme(axis.text = element_blank()) +
  labs(x="", y="",
       title = str_wrap("Cumulative cases of COVID-19 in 2020 per 100,000 population, by Local Authority",width=50),
       subtitle = "",
       caption = paste0(c(areas_relatively_higher_number_cases, source_phs_text), collapse="\n"),
       fill = str_wrap("COVID-19 cases in 2020 per 100,000 population", width = 20)
       ) +
  NULL
...
walk(
  .x = c(".pdf",".svg",".png"),
  .f = ~ggsave(filename = paste0(dir_outputs,"/figure_map_per_capita_covid_cases_2020_la",.x), plot = figure_map_per_capita_covid_cases_2020_la, width = 9, height = 12, dpi = 300)
)
...
figure_map_per_capita_covid_cases_2020_hb <-
  ...
  theme(axis.text = element_blank(), aspect.ratio = 1.4) +
  labs(x="", y="",
       title = str_wrap("Cumulative cases of COVID-19 in 2020 per 100,000 population, by Health Board",width=50),
       subtitle = "",
       caption = paste0(c(areas_relatively_higher_number_cases, source_phs_text), collapse="\n"),
       fill = str_wrap("COVID-19 cases in 2020 per 100,000 population", width = 20)
       ) +
...
walk(
  .x = c(".pdf",".svg",".png"),
  .f = ~ggsave(filename = paste0(dir_outputs,"/figure_map_per_capita_covid_cases_2020_hb",.x), plot = figure_map_per_capita_covid_cases_2020_hb, width = 9, height = 12, dpi = 300)
)
