...
### remotes::install_github("datasciencescotland/opendatascot", force = TRUE)  # this is the old repo
...
files_weekly_deaths$main_report <- "../../../Downloads/weekly-deaths-24-week-17.xlsx"

sheet_cause_of_death_data <- "8"

## updated for 2024, this has all of 2023 plus incomplete 2024 data, with blanks up to week 52
## note that there are fewer columns now because excess deaths have been moved to a separate spreadsheet
# TODO: 2024 has 52 ISOweeks, but 2024-12-30 is W01 of 2025, or the 53rd week of 2024, so this may change
cell_ranges_for_causes_of_death_data <- list(
  "All" = "A6:J110",
  "Care home" = "A113:J217",
  "Home & other non-institution" = "A220:J324",
  "Hospital" = "A327:J431",
  "Other" = "A434:J538"
)
...
weekly_deaths_cause_of_death_2023_onwards <-
  map_dfr(
...
### Data from previous years, 2020-2022
...
```{r}
# TODO: deal with 2022 data here, now that it's 'historical', I load the last known 2023 file that included 2022 data, 2023-07-19_weekly-deaths-23.xlsx
...
weekly_deaths_cause_of_death_2022 <-
  map_dfr(
    .x = cell_ranges_for_causes_of_death_data_2022,
    .f = ~readxl::read_excel(files_historical_deaths$`2022`, sheet = sheet_cause_of_death_data, range = .x), 
    .id = "place_of_death"
  ) %>%
  janitor::clean_names() %>%
  rename(year = registration_year, date_w_c = week_beginning) %>%
...
#     }
#   ) %>% 
#   distinct  # the 2020 and 2021 data get identical historical periodfs & values, so I remove duplicates
```
...
## I treat covid deaths separately to recalculate a realistic estimate of average deaths in prior years
...
weeks_available_2024_disaggregated <- max(merged_deaths_cause_of_death$week_number[merged_deaths_cause_of_death$year==2024])
most_recent_date_available_2024_disaggregated <- max(merged_deaths_cause_of_death$date_w_c)
```
