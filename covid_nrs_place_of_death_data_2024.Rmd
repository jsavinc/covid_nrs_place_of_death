---
title: "Place of death in NRS COVID-19 deaths: data processing for analysis, 2024 data"
author: "Jan Savinc"
...
TODO: now that the ad-hoc files are no longer available, some of the data will need to be compiled from the tabs in the weekly data.

## Overall data

Note: this contains data split by sex and age, but only for ALL places of death; individual PoD data are available for ALL sex and ALL age only
...
weekly_deaths_overall <-
  ...
  filter(!ref_period %in% as.character(2020:2029)) %>% # remove totals for 2020, 2021, 2022, etc., we only want the weekly figures
  parse_ref_period() %>%
  mutate(
...
Note: the last sex & age split of the data provided by NRS was Week 9 of 2023, though the data can still be split using the 'overall' dataset from Open Data Scotland above!

```{r}
# weekly_deaths_sex_age <-
#   weekly_deaths_overall %>%
#   filter(age!="all" & place_of_death != "All")

# TODO: delete the below line, used for testing
# files_weekly_deaths$main_report <- "./downloaded_data/2023-03-22_weekly-deaths-23.xlsx"

# TODO: update this for 2024, probably the same cell ranges as previously?

sheet_covid_deaths_sex_and_age_data <- "1"
sheet_all_cause_deaths_sex_and_age_data <- "4"

# TODO: the data have changed again, now the most recent file no longer includes 2022 and 2023, but only 2023, so the cell ranges below are inaccurate and result in errors!
# 
# cell_ranges_for_sex_and_age_data <- list(
#   "All" = "A6:K110",
#   "Female" = "A113:K217",
#   "Male" = "A220:K324"
# )
...
# TODO: this still doesn't split data by PoD...
weekly_deaths_sex_age <-
  full_join(
    x =
      map_dfr(
        .x = cell_ranges_for_sex_and_age_data,
        .f = ~readxl::read_excel(files_weekly_deaths$main_report, sheet = sheet_covid_deaths_sex_and_age_data, range = .x), 
        .id = "sex"
      ) %>%
      pivot_longer(
        cols = `All ages`:`85+`,
        names_to = "age",
        values_to = "deaths_covid_related"
      ) %>%
      janitor::clean_names(),
    y =
      map_dfr(
        .x = cell_ranges_for_sex_and_age_data,
        .f = ~readxl::read_excel(files_weekly_deaths$main_report, sheet = sheet_all_cause_deaths_sex_and_age_data, range = .x), 
        .id = "sex"
      ) %>%
      pivot_longer(
        cols = `All ages`:`85+`,
        names_to = "age",
        values_to = "deaths_all_causes"
      ) %>%
      janitor::clean_names(),
    by = c("sex", "registration_year", "week_number", "week_beginning", "age")
  ) %>%
  ...
  mutate(
    ...
    deaths_non_covid = deaths_all_causes - deaths_covid_related  # compute non-covid from all-cause minus covid
  ) %>%
  set_covid_and_non_covid_deaths_to_zero_before_reporting_started()

# TODO: this is obsolete now, remove!
...
```{r}
weekly_deaths_la <- 
  read_excel(files_weekly_deaths$la, sheet = "Data", skip = 2) %>% 
  clean_names %>%
...
weekly_deaths_hb <- 
  read_excel(files_weekly_deaths$hb, sheet = "Data", skip = 2) %>% 
  clean_names %>%
...
```{r}
# TODO: need to compute that from another source for the time being
...
# TODO: this is currently commented out as it doesn't compute correctly
...
## updated for 2023 which contains both 2022 and 2023
...
## this was for only 2022 data
## ...and is correct again for 2023
cell_ranges_for_causes_of_death_data <- list(
  "All" = "A6:V58",
  "Care home" = "A61:V113",
...
# TODO: this will eventually only have 2023 data, change when that happens
weekly_deaths_cause_of_death_2022_onwards <-
  ...
  mutate(
    year = as.integer(year),
    measure_type = if_else(condition = str_detect(name, "_five_year_average"), true = "deaths_mean", false = "number_of_deaths"),
  ...
  select(-name) %>%
  pivot_wider(names_from = measure_type, values_from = number_of_deaths)
  # filter(!is.na(number_of_deaths))   # this will also remove the historic mean values, so I've opted not to remove blank entries!
```
...
~~Note that due to how I originally processed these files, the weekly CoD data for 2020 *includes* the 5-year historic weekly average for 2015-2019, whereas I calculate that manually later for the 2021 data and later.~~ This is now changed so that the weekly CoD data is now 
...
```{r}
# TODO: deal with 2022 data here, now that it's 'historical'; this will involve loading the last released data for 2022 released in the weekly releases for 2023, I assume somewhere mid-2023, like it was for 2021 data in 2022 releases
...
# Historic data for deaths

From the [methodology for computing excess deaths during the covid pandemic](https://www.nrscotland.gov.uk/files//statistics/covid19/covid-deaths-methodology-excess-deaths.pdf):
...
Similarly for 2023, the five year average will again leave out 2020 and will be built from the data from 2017, 2018, 2019, 2021 and 2022.

'Historical' here refers to the 5-year period preceding each year, useful for a historic comparison. The historic period depedning on year has been defined as:

| year|historic_period      |
|----:|:--------------------|
| 2020|2015-2019            |
| 2021|2015-2019            |
| 2022|2016-2019, 2021      |
| 2023|2017-2019, 2021-2022 |


There is a second sense of 'historical' here, which denotes cause of death figures for 2020 to 2022; these were only provided as 5-year averages for 2015-2019, whereas we have a per-cause breakdown from 2020 onwards - to combine these, a 5-year historical average needs to be recomputed using a combination of the 2015-2019 data and the 2020+ data (using weighted averages). This is done in [the later section recomputing historical averages for causes of death](#historical-annual-average-by-cause).
...
Since 2021, the 5-year historic context has been amended to exclude 2020 (see [above section on historical cause of death data](#historical-causes-of-death)). Because of this, the weekly average needs to be recalculated depending on which historic period is of interest.

## Historic periods by year

This is a reference table for the year of data, its historic period, and a nested list of vectors of years of historical data.

```{r}
historic_period_by_year <-
  tibble(year = 2020:latest_year) %>%
  mutate(
    historic_period = historic_period_depending_on_year(year),
    years_in_historic_period = map(.x = as.integer(str_sub(historic_period, 1, 4)), .f = generate_5_years_historic_period_excl_2020)
    )
```


## Historic averages, overall

```{r, warning=FALSE}
## helper functions to filter historic data for different 
get_historic_and_current_data_from_year_range <- function(historic_data, current_data, year_range) {
  year_range <- year_range[year_range!=2020]  # remove 2020 from range
  historic_range <- year_range[year_range < 2020]
  current_range <- year_range[year_range >= 2020]
  bind_rows(
    historic_data %>% 
      filter(year %in% historic_range),
    current_data %>% 
      filter(year %in% current_range) %>% 
      rename(number_of_deaths = deaths_all_causes)  # also rename to historic data format
  )
}

historical_weekly_average_overall <-
  pmap_dfr(
    .l = historic_period_by_year,
    .f = function(year, historic_period, years_in_historic_period) {
      get_historic_and_current_data_from_year_range(
        historic_data = 
          weekly_deaths_2015_2019_overall %>%
          filter(year!="average") %>%
        ...
        year_range = years_in_historic_period
      ) %>%
      ...
      mutate(historic_period = historic_period)
    }
  ) %>%
  distinct()
...
## Historic averages, HB, LA, sex, age

```{r}
## helper function to summarise with mean, min, and max
...
historical_weekly_average_hb <-
  pmap_dfr(
    ...
    .f = function(year, historic_period, years_in_historic_period) {
      get_historic_and_current_data_from_year_range(
        historic_data = weekly_deaths_2015_2019_hb, 
  ...
  distinct  # the 2020 and 2021 data get identical historical periods & values, so I remove duplicates
...
historical_weekly_average_la <-
  pmap_dfr(
    ...
    .f = function(year, historic_period, years_in_historic_period) {
      get_historic_and_current_data_from_year_range(
        historic_data = weekly_deaths_2015_2019_la, 
...
historical_weekly_average_sex_age <-
  pmap_dfr(
    ...
    .f = function(year, historic_period, years_in_historic_period) {
      get_historic_and_current_data_from_year_range(
        historic_data = weekly_deaths_2015_2019_sex_age, 
...
historical_weekly_average_age <-
  pmap_dfr(
    ...
    .f = function(year, historic_period, years_in_historic_period) {
      get_historic_and_current_data_from_year_range(
        historic_data = weekly_deaths_2015_2019_age, 
...
historical_weekly_average_sex <-
  pmap_dfr(
    ...
    .f = function(year, historic_period, years_in_historic_period) {
      get_historic_and_current_data_from_year_range(
        historic_data = weekly_deaths_2015_2019_sex, 
...
## Historic averages, annual average

Here I'm computing the 5-year annual average for various groupings of variables, and across the various possible historic periods. Instead of taking the weekly averages for a historic period computed in the above section, I recompute this separately from the raw data for historic periods and current data.

```{r}
## helper function to compute annual average per historic period, per place of
## death, and per various grouping variables as needed
...
## Note, there was an error in these before, where I erroneously omitted 2015
## figures from the annual averages computation for 2015-2019; this had minimal
## effect on the mean/min/max values because the algorithm used averaged over
## the available years (as opposed to e.g. dividing 4 years of data by 5)

# the below commented out section is no longer used, but I've left it to
# demonstrate the error in previous data - I think I started 'splitting' the
# 2015-2019 afrom the 2016-2021 historic periods but didn't end up implementing
# both, and instead left a confusing entry that compute the 2016-2019 period
# only.
...
historical_annual_average_overall <-
  pmap_dfr(
    ...
    .f = function(year, historic_period, years_in_historic_period) {
      get_historic_and_current_data_from_year_range(
        historic_data = 
          weekly_deaths_2015_2019_overall %>%  # a quirk of the weekly deaths 'overall' data
          filter(year != "average") %>%
          mutate(year = as.integer(year)), 
        current_data = 
          weekly_deaths_overall %>%
  ...
  distinct %>%  # 2020 and 2021 use the same historic period, so I remove duplicate entries at this point
...
# Historic average by cause of death

Cause of death data comes on a per-year basis, with the current year's data including the preceding year; at some point mid-year, this changes, and only the current year's data are included, and the preceding year's data need to be loaded from the last release that included them. (e.g. mid-2022, causes of death for 2021 stopped being released alongside 2022 data, so I load them from the last 2022 release that included 2021 causes of death also). They therefore come as one dataset per year, with the current year as a separate dataset. The weekly figures for cause of death are included alongside the 5-year historic average for that week except for Covid, which has been set to NA for all data up and including 2023 data as of February 2023.
...
```{r}
# TODO: include 2023 data once it's reported separately

# TODO: I still need to separately process the most recent year because the average values aren't provided

## here I separately merge the datasets for cause of death data from different years
## I treat covid deaths separately to recalculate a realistic esstimate of average deaths in prior years
weekly_deaths_cause_of_death_noncovid <-
  bind_rows(
    weekly_deaths_cause_of_death_2020,
    weekly_deaths_cause_of_death_2021,
    weekly_deaths_cause_of_death_2022_onwards
...
weekly_deaths_cause_of_death_covid_before_processing <-
  bind_rows(
    weekly_deaths_cause_of_death_2020,
    weekly_deaths_cause_of_death_2021,
    weekly_deaths_cause_of_death_2022_onwards
...
weekly_deaths_cause_of_death_covid <-
  weekly_deaths_cause_of_death_covid_before_processing %>%
  filter(cause_of_death == "COVID-19") %>%
  group_by(year) %>%
  group_map(
    .f = function(data_tbl, year_tbl) {
      current_year <- year_tbl$year  # get year from the function arguments
      year_range <- infer_year_range_for_calculating_covid_deaths(current_year)  # work out which years of data to use
      weekly_means <-  
        weekly_deaths_cause_of_death_covid_before_processing %>%  # bind again weekly deaths
        filter(year %in% year_range) %>%  # filter the covid data to the relevant years to compute from
        create_week_53_data_for_years_with_52_weeks() %>%  # create 53rd week if needed
        group_by(place_of_death, cause_of_death, week_number) %>%
        summarise(
          deaths_mean = mean(number_of_deaths, na.rm=TRUE),
          # year_range = paste0(year_range,collapse=","),  # for testing
          .groups = "drop"
        )
      data_tbl %>%
        mutate(year = current_year) %>%  # this gets removed in the grouping, so I re-add it here
        select(-deaths_mean) %>%  # remove the NA/0 mean deaths
        create_week_53_data_for_years_with_52_weeks() %>%  # create 53rd week if needed
        left_join(weekly_means, by = c("place_of_death", "cause_of_death", "week_number"))  # merge the mean deaths
    }
  ) %>%
  bind_rows()  # merge into single tibble from list

## re-merge non-covid and covid deaths again
# TODO: make CoD, PoD factors
...
## note that these might differ from actual historic figures a bit because of
## my hack to create a fake 53rd week in years with only 52 weeks for easier 
## comparison

# TODO: I get different numbers of 5-year annual average deaths between 2020 and 2021, how come?
historical_annual_average_cause_of_death <-
  weekly_deaths_cause_of_death %>%
  group_by(year, historic_period, place_of_death, cause_of_death) %>%
  summarise(
    annual_deaths_mean = sum(deaths_mean),
    .groups = "drop"
  ) %>% 
  filter(year!=2020) %>%  # 2020 and 2021 should have identical annual means so I remove one %>%
  select(-year)

## checking discrepancy:
## I get different numbers of 5-year annual average deaths between 2020 and 2021, how come?
weekly_deaths_cause_of_death %>% filter(year %in% 2020:2021) %>% select(place_of_death, cause_of_death, year, week_number, deaths_mean) %>%
  pivot_wider(names_from = year, values_from = deaths_mean) %>%
  filter(`2020`!=`2021`) %>%
  arrange(desc(week_number)) %>%
  summarise(across(matches("\\d{4}"), ~sum(.x, na.rm=TRUE)))
## looks like 2020 and 2021 don't actually use the same 5-year weekly average deaths by cause!!!
# TODO: act on this knowledge...
```
...
## Note: previously I computed the historical average separately fro mthe weekly cause of death data
## Now it's already computed in the same dataset, so I just assign it here, no joining needed
...
These are used for saying in plots what data were included e.g. weeks 1-12 in 2023.

There is as of April 2023 a split in the availability of data:

- Data are being continually updated for the all age all sex all HB/LA dataset
- Data split by age, sex, HB or LA stopped being updated (technically they are still available but not without place of death data)

Therefore there are two separate most recent available data, I'll call them 'basic' and 'disaggregated'/

```{r}
## constants for use in graphs & annotations
weeks_available_2024 <- max(merged_deaths_overall$week_number[merged_deaths_overall$year==2024])
most_recent_date_available_2024 <- max(merged_deaths_overall$date_w_c)

earliest_date <- min(merged_deaths_overall$date_w_c)
current_year <- isoyear(most_recent_date_available_2024)

weeks_available_2024_disaggregated <- max(merged_deaths_age$week_number[merged_deaths_age$year==2024])
most_recent_date_available_2024_disaggregated <- max(merged_deaths_age$date_w_c)
```
...
table_current_deaths_compared_to_past <-
  bind_rows(
    ...
    merged_deaths_overall %>%
      ...
      summarise(
        period = glue::glue("Data from week 1, 2020 to week {weeks_available_2024}, 2022"),
    ...
    group_by(place_of_death) %>%
    summarise(
      period = glue::glue("2024, up to week {weeks_available_2024}"),
      historic_period = "2015-2019",
      deaths_all_causes = sum(deaths_all_causes),
      deaths_non_covid = sum(deaths_non_covid, na.rm = TRUE),
      deaths_covid_related = sum(deaths_covid_related, na.rm = TRUE),
...
merged_proportions_of_deaths_by_place <-
  ...
  mutate(place_of_death = factor(place_of_death, levels = order_of_place_of_death_levels[c(1,3,2,4,5)])) %>%
  arrange(year, place_of_death)

write_csv(x = merged_proportions_of_deaths_by_place, file = file.path(dir_outputs, "annual_deaths_by_place.csv"))
```
...
# Causes of death over time, proportion inside and between places

Table of number of deaths by cause, place of death and year, showing proportion of deaths by cause within each place `prop_cod_within_pod` (e.g. Covid deaths were 2% of home deaths) and proportion of deaths by place within each cause `prop_pod_within_cod` (e.g. 70% of all Covid deaths happened in hospital).
Note that because historic data was given averaged over the 5 year period, the historic figures are given as a mean over the period rather than an annual total.

```{r}
table_proportions_of_cause_of_death <-
  bind_rows(
    ## historic periods
    historical_annual_average_cause_of_death %>%
      rename(year = historic_period, deaths = annual_deaths_mean),
    ## current data
    total_deaths_cause_of_death %>%
      select(-c(historic_period,annual_deaths_mean:ncol(.))) %>%
      mutate(year = as.character(year))
  ) %>%
  filter(place_of_death!="All" & cause_of_death!="All") %>%
  group_by(year,place_of_death) %>%
  mutate(prop_cod_within_pod = deaths / sum(deaths)) %>%
  group_by(year,cause_of_death) %>%
  mutate(prop_pod_within_cod = deaths / sum(deaths)) %>%
  ungroup %>%
  (function(data_tbl){
    bind_rows(
      data_tbl,
  ...
  mutate(
    ...
    cause_of_death = factor(cause_of_death, levels = order_of_causes_of_death),
    place_of_death = factor(place_of_death, levels = order_of_place_of_death_levels[c(5,1,3,2,4)])
    ) %>%
  arrange(year, place_of_death, cause_of_death)

table_proportions_of_cause_of_death %>%
  write_csv(file = paste0(dir_outputs,"/proportions_of_deaths_by_place_&_cause.csv"))
```
