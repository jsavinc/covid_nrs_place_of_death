---
title: "Place of death in NRS COVID-19 deaths: visualisation for 2024 data"
author: "Jan Savinc"
...
date_range_text <- glue::glue("Data from w/c {format(earliest_date, '%d %B %Y')} to w/c {format(most_recent_date_available_2024, '%d %B %Y')} (inclusive).")
date_range_text_disaggregated <- glue::glue("Data from w/c {format(earliest_date, '%d %B %Y')} to w/c {format(most_recent_date_available_2024_disaggregated, '%d %B %Y')} (inclusive).")
...
only_2024_text <- glue::glue("Data for 2024 only include weeks 1-{format(weeks_available_2024)} (w/c {format(most_recent_date_available_2024)}).")
only_2024_disaggregated_text <- glue::glue("Data for 2024 only include weeks 1-{format(weeks_available_2024_disaggregated)} (w/c {format(most_recent_date_available_2024_disaggregated)}).")
...
fig_place_of_death <-
  ...
  geom_vline(xintercept = compute_start_date_from_week_number(week_number = 1, year_number = 2023), linetype = "dashed", colour="grey50") +  # start of 2023
  geom_vline(xintercept = compute_start_date_from_week_number(week_number = 1, year_number = 2024), linetype = "dashed", colour="grey50") +  # start of 2024
  scale_x_date(date_labels = "%d-%m-%y", date_breaks = "12 weeks", limits = c(earliest_date, most_recent_date_available_2024)) +
  ...
  annotate(geom = "text", x=ymd("2021-01-07"), y=30, label = "'21", hjust = 0, colour = "grey40") +
  annotate(geom = "text", x=ymd("2022-01-07"), y=30, label = "'22", hjust = 0, colour = "grey40") +
  annotate(geom = "text", x=ymd("2023-01-02"), y=30, label = "'23", hjust = 0, colour = "grey40") +
  annotate(geom = "text", x=ymd("2024-01-01"), y=30, label = "'24", hjust = 0, colour = "grey40") +
  NULL
...
fig_place_of_death_2015_2019_range <-
  merged_deaths_overall_2015_2019_historic_period %>%
  filter(sex == "all" & age == "all") %>%
  filter(!place_of_death %in% c("All","Other")) %>%
  mutate(place_of_death = factor(place_of_death, levels = c("Hospital", "Home & other non-institution", "Care home"))) %>%  # needs to be factor with same levels, otherwise it messes up level order when added as geom_text later!)
...
fig_place_of_death_2015_2019_range %>%
save_output_file(filename = file.path(dir_outputs, paste0(c("fig_place_of_death_2015_2019_range", latest_data_modified_date), collapse = "_")), extensions = c(".pdf",".png"), plot = ., width = 28, height = 14, dpi = 300, units = "cm")
```
...
# Number & Proportion annual deaths by place
...
(fig_proportion_annual_deaths_by_place <-
  ...
  labs(
    ...
    colour = NULL,
    subtitle = paste0("* ",only_2024_text),
...
```{r}
(fig_annual_deaths_by_place <-
  wrap_plots(
    # Number
    merged_proportions_of_deaths_by_place %>%
    ggplot(data = ., 
           aes(x = year, y = number_of_deaths, colour = place_of_death, shape = place_of_death, linetype = place_of_death)
           ) +
    geom_line(linewidth = 1) +
    geom_point(size = rel(2)) +
    geom_text(data = ~filter(.x, year==latest_year), aes(x=latest_year+0.2, y=number_of_deaths, label="*"), inherit.aes = FALSE) +
    theme_minimal() +
    theme(legend.position = "top", legend.title = element_blank(), axis.text.x = element_text(angle = 45, hjust = 1)) +
    scale_x_continuous(breaks = sort(unique(merged_proportions_of_deaths_by_place$year)), minor_breaks = NULL) +
    scale_y_continuous(labels = scales::comma, breaks = seq.int(from=0, to=7e4, by=1e4), minor_breaks = seq.int(from=0,t=7e4,by=5e3)) +
    scale_colour_manual(values = c(scales::viridis_pal(option = "C")(4),"grey")) +
    scale_shape_manual(values = c(scales::shape_pal()(4),NA)) +
    ...
    filter(place_of_death!="All") %>%
    ggplot(data = ., 
           aes(x = year, y = proportion_of_total, colour = place_of_death, shape = place_of_death)
           ) +
    geom_line(linewidth = 1) +
    geom_point(size = rel(2)) +
    geom_text(data = ~filter(.x, year==latest_year), aes(x=latest_year+0.2, y=proportion_of_total, label="*"), inherit.aes = FALSE) +
   ...
   plot_annotation(theme = theme(legend.position = "top"), subtitle = paste0("* ",only_2024_text),
    caption = paste0(c(death_registrations_text,source_nrs_text),collapse="\n"))
 )

fig_annual_deaths_by_place %>%
save_output_file(filename = file.path(dir_outputs, paste0(c("fig_annual_deaths_by_place", latest_data_modified_date), collapse = "_")), extensions = c(".pdf",".png"), plot = ., width = 20, height = 10, dpi = 300, units = "cm")
```
...
(fig_deaths_so_far_by_cause <-
  ...
  labs(
    ...
    subtitle = paste0(c("Bars are no. of deaths, line segments are mean annual deaths 2015-2019", paste0("* ",only_2024_text)), collapse="\n"),
    caption = paste0(c(death_registrations_text, source_nrs_text),collapse="\n")
  ) +
  NULL)
...
fig_cause_of_death_annual_proportion_by_place <-
  ...
  geom_line() +
  geom_point() +
  geom_text(data = ~filter(.x, year==as.character(latest_year)), aes(x=n_distinct(current_and_historical_deaths_by_cause_and_location$year)+0.2, y=proportion, label="*"), inherit.aes = FALSE) +  # cobble together the final year position from the data!
...
(fig_home_deaths_over_time_by_hb_4_week_average <-
  ...
  # geom_ribbon(aes(ymin = deaths_min, ymax = deaths_max), fill = "#4477aa", alpha = 0.5) +
  geom_line(aes(y = deaths, colour = deaths_type)) +
  geom_line(aes(y = ma4w_deaths_mean, linetype = "dashed"), linewidth = 0.2) +
  geom_vline(xintercept = compute_start_date_from_week_number(week_number = 1, year_number = 2021), linetype = "dashed", colour="grey50") +  # start of 2021
  geom_vline(xintercept = compute_start_date_from_week_number(week_number = 1, year_number = 2022), linetype = "dashed", colour="grey50") +  # start of 2022
...
(fig_proportion_of_deaths_by_place_by_hb <-
  ...
  geom_line(aes(linewidth = place_of_death)) +
  geom_text(data = ~filter(.x, year==latest_year), aes(x=latest_year+0.2, y=proportion_of_total, label="*"), inherit.aes = FALSE) +
  ...
  labs(
    ...
    subtitle = paste0("* ",only_2024_disaggregated_text),
    caption = paste0(c("Health boards ordered by 2020 population estimate.",death_registrations_text,source_nrs_text),collapse="\n")
    ) +
  NULL)
...
## Absolute num. home deaths by LA

```{r}
# TODO: remove theme_minimal when done testing
# TODO: add Scotland graph also
# TODO: check  if all PoD would work?

la_group <- tibble(  # group LAs into 4 groups of 8
  ref_area = factor(order_la_by_population),
  # la_group = rep(1:4, each = 8))
  la_group = rep(1:6, times = c(6,5,5,5,5,6))
  ) %>%
  group_by(la_group) %>%
  mutate(la_group_label = paste0(ref_area[1], " - ", ref_area[n()])) %>%
  ungroup %>%
  mutate(la_group_label = factor(la_group_label, levels = unique(la_group_label)))
  

(fig_number_of_deaths_by_place_by_la <-
  proportions_of_deaths_by_place_by_geography %>%
  filter(geography == "la") %>%
  filter(place_of_death == "Home & other non-institution") %>%
  left_join(la_group, by = "ref_area") %>%  # group LAs
  mutate(ref_area = factor(x = ref_area, levels = order_la_by_population)) %>%  # arrange la/la by population size
  ggplot(. , aes(x=year, y=number_of_deaths, colour=ref_area, shape = ref_area)) +
    theme_minimal() +
    geom_point(size = rel(2)) +
    geom_line() +
    geom_text(data = ~filter(.x, year==latest_year), aes(x=latest_year+0.2, y=number_of_deaths, label="*"), inherit.aes = FALSE) +
    facet_rep_wrap(~ la_group_label, repeat.tick.labels = TRUE, scales = "free_y") +
    scale_x_continuous(breaks = sort(unique(proportions_of_deaths_by_place_by_geography$year))) +
    scale_y_continuous(label = scales::comma) +
    scale_shape_manual(values = map(c(6,5,5,5,5,6), scales::shape_pal()) %>% unlist) +
    scale_colour_manual(values = map(c(6,5,5,5,5,6), viridis::viridis_pal()) %>% unlist) +
    theme(
      legend.position = "top",
      legend.title = element_blank(),
      legend.text = element_text(size = 12),
    ...
    guides(
      colour = guide_legend(ncol = 6, title = NULL), 
      shape = guide_legend(ncol = 6, title = NULL)
      ) +
    labs(
      x = NULL, y = NULL,
      title = "Deaths at home by LA",
  subtitle = paste0("* ",only_2024_disaggregated_text),
  caption = paste0(c("Local Authorities ordered by 2020 population estimate.",death_registrations_text,source_nrs_text),collapse="\n")
      ) +
    NULL
...
fig_number_of_deaths_by_place_by_la %>%
save_output_file(filename = file.path(dir_outputs, paste0(c("fig_number_of_deaths_by_place_by_la", latest_data_modified_date), collapse = "_")), extensions = c(".pdf",".png"), plot = ., width = 30, height = 25, dpi = 300, units = "cm")
```
...
(fig_map_home_deaths_increase_from_2020_la <-
  ...
  labs(x="", y="",
       ...
       caption = paste0(c(only_2024_disaggregated_text,source_nrs_text), collapse="\n"),
       fill = "Home death\nincrease relative\nto 2015-2019"
       ) +
  NULL)
...
(fig_map_covid_cases_from_2020_la <-
  ...
  labs(x="", y="",
       ...
       subtitle = "",
       caption = paste0(c(only_2024_disaggregated_text,crude_rate_text,source_phs_text), collapse="\n"),
...
# Years 'skipped ahead' due to Covid

There was a pre-pandemic trend of increasing deaths at home - by plotting a regression line we can compute the 'years skipped ahead' equivalent to the increase in deaths at home due to covid
...
regression_model <- lm(data = regression_data, formula = number_of_deaths ~ years_since_2015)

predicted_data <- tibble(years_since_2015 = seq(from=0, to=30, by = 0.1)) %>%
  mutate(year = 2015 + years_since_2015,
         predicted = predict.lm(object = regression_model, newdata = tibble(years_since_2015), type = "response")
         )

## 2022 only has X weeks' worth of data available; we can extrapolate the annual
...
extrapolated_2020_data <-
  historical_weekly_average_overall %>%
  filter(place_of_death == "Home & other non-institution" & historic_period == "2015-2019") %>%
  summarise(
    deaths_mean_so_far = sum(deaths_mean[week_number <= weeks_available_2024], na.rm= TRUE),
    deaths_mean_total = sum(deaths_mean, na.rm= TRUE),
    proportion_of_deaths_so_far = deaths_mean_so_far / deaths_mean_total
    )

actual_and_predicted_data <-
  bind_rows(
    merged_proportions_of_deaths_by_place %>%
    ...
    mutate(
      data_type = "Predicted",
      number_of_deaths = if_else(year %in% 2020:current_year, compute_y_from_x_using_linear_regression(x = year-2015, regression_model = regression_model), number_of_deaths)
      )
    ,
    merged_proportions_of_deaths_by_place %>%
      select(year, number_of_deaths, place_of_death) %>%
      filter(place_of_death == "Home & other non-institution" & year %in% 2020:current_year) %>%
      mutate(
        number_of_deaths = if_else(year == current_year, number_of_deaths / extrapolated_2020_data$proportion_of_deaths_so_far, number_of_deaths),
        year = 2015 + compute_x_from_y_using_linear_regression(y = number_of_deaths, regression_model = regression_model),  # compute predicted X
...
## define the limites of the rectangles used to highlight areas of the graph
regression_rectangles <-
  actual_and_predicted_data %>% 
  filter(data_type %in% c("Predicted","Actual")) %>%
  group_by(data_type) %>%
  summarise(year_min = min(year), year_max = max(year), deaths_min = min(number_of_deaths), deaths_max = max(number_of_deaths))

## annotations to go above the rectangles
predicted_data_annotations <- tribble(
  ~x, ~y, ~label,
  rowMeans(regression_rectangles[1,c("year_min","year_max")]), regression_rectangles[[1,"deaths_max"]]+2000, glue::glue("Predicted home deaths 2020-{current_year} using 2015-2019 data"),
  rowMeans(regression_rectangles[2,c("year_min","year_max")]), regression_rectangles[[2,"deaths_max"]]+2000, glue::glue("Actual home deaths 2020-{current_year}*"),
  rowMeans(regression_rectangles[2,c("year_min","year_max")]), regression_rectangles[[2,"deaths_min"]]-3000, "If the 2015-2019 trend continued we wouldn't observe this number of home deaths until 2031",
)

fig_expected_home_deaths_regression <-
  ggplot(data = actual_and_predicted_data, aes(x=year, y=number_of_deaths)) +
  # highlighting predicted data for 2020 onwards
  statebins:::geom_rrect(aes(xmin = regression_rectangles[[1,"year_min"]]-0.5, xmax = regression_rectangles[[1,"year_max"]]+0.5, ymin = regression_rectangles[[1,"deaths_min"]]-1000, ymax = regression_rectangles[[1,"deaths_max"]]+1000), fill = alpha("grey",0), colour = "red") +
  # highlighting actual data
  statebins:::geom_rrect(aes(xmin = regression_rectangles[[2,"year_min"]]-0.5, xmax = regression_rectangles[[2,"year_max"]]+0.5, ymin = regression_rectangles[[2,"deaths_min"]]-1000, ymax = regression_rectangles[[2,"deaths_max"]]+1000), fill = alpha("grey",0), colour = "orange") +
  geom_text(data = predicted_data_annotations, aes(x=x,y=y,label=str_wrap(label, width = 60))) +
  geom_text(data = actual_and_predicted_data %>% filter(actual_year %in% 2020:current_year), aes(x = year, y = number_of_deaths + 500, label = if_else(actual_year==current_year, paste0(actual_year,"*"), as.character(actual_year))), angle = 30) +
  geom_point(aes(shape = data_type)) +
  geom_line(data = predicted_data %>% filter(year <= ceiling(max(regression_rectangles$year_max))), aes(x = year, y = predicted), linetype = "dotted") +
  theme_minimal() +
  theme(legend.position = "top", legend.title = element_blank()) +
  scale_y_continuous(labels = scales::comma) +
  scale_x_continuous(breaks = 2015:2045, minor_breaks = NULL, limits = c(2015,ceiling(max(regression_rectangles$year_max)))) +
  theme(axis.text.x = element_text(size = rel(0.8), angle = 45, hjust = 1)) +
  labs(y = NULL, x = "Year (hypothetical)", title = "Increasing deaths at home", subtitle = str_wrap("Home deaths were on the increase before the pandemic.\nIf we assumed the same annual increases from 2015-2019 we wouldn't expect the pandemic levels of home deaths until approx. 2031!", width = 90), caption = paste0(c(glue::glue("*Annual home deaths for 2024 extrapolated from data for weeks 1-{weeks_available_2024}"),source_nrs_text), collapse="\n"))

fig_expected_home_deaths_regression %>%
save_output_file(filename = file.path(dir_outputs, paste0(c("fig_expected_home_deaths_regression", latest_data_modified_date), collapse = "_")), extensions = c(".pdf",".png"), plot = ., width = 18, height = 12, dpi = 300, units = "cm")
```
