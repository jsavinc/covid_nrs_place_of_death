...
## What does the first letter in the `location code` mean?

I suspect `B` is for Borders HB...

```{r}
locations_hb_borders_other %>% count(str_sub(location_code,1,1))


locations %>% anti_join(locations_hb_borders_other) %>% count(str_sub(location_code, 1, 1))

locations %>% anti_join(locations_hb_borders_other) %>% filter(str_sub(location_code, 1, 1) == "B") %>% count(is.na(postcode))

locations %>% anti_join(locations_hb_borders_other) %>% filter(str_sub(location_code, 1, 1) == "B")
```
