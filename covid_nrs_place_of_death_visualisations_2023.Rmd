...
predicted_data <- tibble(years_since_2015 = seq(from=0, to=30, by = 0.1)) %>%
  mutate(year = 2015 + years_since_2015,
         predicted = predict.lm(object = regression_model, newdata = tibble(years_since_2015))
         )
...
actual_and_predicted_data <-
  bind_rows(
    ...
    mutate(
      data_type = "Predicted",
      number_of_deaths = if_else(year %in% 2020:current_year, compute_y_from_x_using_regression(x = year-2015, regression_model = regression_model), number_of_deaths)
      )
    ,
    merged_proportions_of_deaths_by_place %>%
      select(year, number_of_deaths, place_of_death) %>%
      ...
      mutate(
        ...
        year = 2015 + compute_x_from_y_using_regression(y = number_of_deaths, regression_model = regression_model),  # compute predicted X
