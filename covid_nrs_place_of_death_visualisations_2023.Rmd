...
extrapolated_2020_data <-
  ...
  summarise(
    deaths_mean_so_far = sum(deaths_mean[week_number <= weeks_available_2022], na.rm= TRUE),
...
predicted_data_annotations <- tribble(
  ~x, ~y, ~label,
  2021, 20e3, "Predicted home deaths 2020-2022 using 2015-2019 data",
  2033, 23e3, "Actual home deaths 2020-2022*",
  2033, 17.5e3, "If the 2015-2019 trend continued we wouldn't observe this number of home deaths until 2031",
)
...
actual_and_predicted_data <-
  bind_rows(
    ...
    merged_proportions_of_deaths_by_place %>%
    select(year, number_of_deaths, place_of_death) %>%
    filter(place_of_death == "Home & other non-institution" & year %in% 2020:2022) %>%
    mutate(
      data_type = "Predicted",
      number_of_deaths = if_else(year %in% 2020:2022, compute_y_from_x_using_regression(x = year-2015, regression_model = regression_model), number_of_deaths)
      )
    ,
    merged_proportions_of_deaths_by_place %>%
      select(year, number_of_deaths, place_of_death) %>%
      filter(place_of_death == "Home & other non-institution" & year %in% 2020:2022) %>%
      mutate(
        number_of_deaths = if_else(year == 2022, number_of_deaths / extrapolated_2020_data$proportion_of_deaths_so_far, number_of_deaths),
        year = 2015 + compute_x_from_y_using_regression(y = number_of_deaths, regression_model = regression_model),  # compute predicted X
        data_type = "Actual"  # note: this is actual figures but
      )
  )

fig_expected_home_deaths_regression <-
  ggplot(data = actual_and_predicted_data, aes(x=year, y=number_of_deaths)) +
  geom_rect(aes(xmin = 2019.5, xmax = 2022.5, ymin = 15.5e3, ymax = 18.5e3), fill = "yellow", alpha = 0.3) +
  geom_rect(aes(xmin = 2030, xmax = 2035, ymin = 19.5e3, ymax = 22e3), fill = "yellow", alpha = 0.3) +
  geom_text(data = predicted_data_annotations, aes(x=x,y=y,label=str_wrap(label, width = 40))) +
  geom_point(aes(shape = data_type)) +
  geom_line(data = predicted_data, aes(x = year, y = predicted), linetype = "dotted") +
  theme_minimal() +
  theme(legend.position = "top", legend.title = element_blank()) +
  xlim(c(2015,2037)) +
  scale_y_continuous(labels = scales::comma) +
  labs(y = NULL, x = "Year (hypothetical)", title = "Increasing deaths at home", subtitle = str_wrap("Home deaths were on the increase before the pandemic.\nIf we assumed the same annual increases from 2015-2019 we wouldn't expect the pandemic levels of home deaths until 2031!", width = 90), caption = paste0(c(glue::glue("*Annual home deaths for 2022 extrapolated from data for weeks 1-{weeks_available_2022}"),source_nrs_text), collapse="\n"))
