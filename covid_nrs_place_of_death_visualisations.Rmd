...
## Table of annual deaths by cause
...
(figure_annual_deaths_by_cause <-
  ...
  ggplot() +
  # geom_col(aes(x = cause_of_death, fill = cause_of_death, y = deaths, group = deaths_type, alpha = deaths_type), position = "dodge", colour = "grey10") +
  geom_col(aes(x = cause_of_death, fill = cause_of_death, y = deaths), alpha = 0.8, colour = "grey10") +
  # geom_point(aes(x = cause_of_death, y = annual_deaths_mean)) +
  geom_segment(aes(x = as.numeric(cause_of_death)-0.45, xend = as.numeric(cause_of_death)+0.45, y = annual_deaths_mean, yend = annual_deaths_mean), size = 1) +
  coord_flip() +
  scale_fill_viridis_d() +
  facet_wrap(~place_of_death) +
  # scale_alpha_manual(
  #   values = c(0.4,1), labels = c("Historic mean", "2020 deaths")
  # ) +
  scale_y_continuous(labels = function(x) format(x, big.mark = ",")) +
  theme(
    # legend.position = "top", 
  ...
  labs(
    x = NULL,
    y = "Deaths",
    # subtitle = "Bars are 2020 deaths, dots are (•) mean annual deaths 2015-2019"
    subtitle = "Bars are 2020 deaths, line segments are mean annual deaths 2015-2019"
  ) +
  NULL
)

save_output_file(filename = paste0(dir_outputs,"/figure_annual_deaths_by_cause"), extensions = c(".pdf",".svg",".png"), plot = figure_annual_deaths_by_cause, width = 12, height = 6, dpi = 300, units = "in")
```
...
(figure_deaths_at_home_by_cause <-
  ...
  geom_line(aes(y = number_of_deaths, linetype = deaths_type)) +
