...
(figure_deaths_at_home_2020_and_2021 <-
  ...
  geom_ribbon(aes(ymin = deaths_min, ymax = deaths_max), fill = "#4477aa", alpha = 0.5) +
  geom_line(aes(y = deaths_mean), linetype = "dashed", colour="grey20") +
  # geom_line(aes(y = deaths, colour = deaths_type, alpha = year)) +
  geom_line(data = . %>% filter(year==2020), aes(y = deaths, colour = deaths_type, alpha = week_number), show.legend = FALSE) +
  ...
  annotate(geom = "text", x = 18, y = 550, label = "2020", alpha = 0.6) +
...
(figure_deaths_by_cause <-
  ...
  labs(
    x = NULL,
    y = "Deaths",
    subtitle = paste0(c(paste0("Bars are deaths ",str_remove_all(date_range_text,"Data ")),"Line segments are mean annual deaths 2015-2019"),collapse="\n")
  ) +
  NULL
)
