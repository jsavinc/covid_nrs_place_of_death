...
### Deaths by place, both years


```{r}
annotations_deaths_by_place_2020_and_2021 <- tribble(
  ~place_of_death, ~x, ~y, ~label,
  "Hospital", 4, 400, 2021,
  "Hospital", 18, 800, 2020,
  "Home & other non-institution", 4, 540, 2021,
  "Home & other non-institution", 18, 610, 2020,
  "Care home", 4, 160, 2021,
  "Care home", 11, 600, 2020
) %>% mutate(alpha = if_else(label==2021, 1, 0.4)) %>%   # change alpha for 2020
  mutate(place_of_death = factor(place_of_death, levels = c("Hospital", "Home & other non-institution", "Care home")))  # needs to be factor with same levels, otherwise it messes up level order when added as geom_text later!)

arrows_deaths_by_place_2020_and_2021 <- tribble(
   ~geom, ~place_of_death, ~label, ~x, ~y, ~xend, ~yend,
  "text", "Home & other non-institution", "Historic range (min & max)", 15, 90, NA, NA,
  "curve", "Home & other non-institution", NA, 10, 110, 14, 250,
  "text", "Home & other non-institution", "Historic mean", 34, 40, NA, NA,
  "curve", "Home & other non-institution", NA, 34, 60, 39, 275
) %>% 
  mutate(place_of_death = factor(place_of_death, levels = c("Hospital", "Home & other non-institution", "Care home")))  # needs to be factor with same levels, otherwise it messes up level order when added as geom_text later!)

(figure_deaths_by_place_2020_and_2021 <-
  merged_deaths_overall %>%
  filter(sex == "all" & age == "all") %>%
  filter(!place_of_death %in% c("All","Other")) %>%
  select(-c(date_w_c, week_number_run_over, ref_period, ref_area)) %>%
  ...
  pivot_longer(cols = c(deaths_all_causes, deaths_non_covid),
               names_to = "deaths_type", values_to = "deaths") %>%
  ggplot(aes(x = week_number)) +
  geom_ribbon(aes(ymin = deaths_min, ymax = deaths_max), fill = "grey", alpha = 0.3) +
  geom_line(aes(y = deaths_mean), linetype = "dashed", colour="grey20") +
  geom_line(data = . %>% filter(year==2020), aes(y = deaths, colour = deaths_type, group = deaths_type), show.legend = FALSE, alpha = 0.5) +  # use a fixed alpha value, otherwise the lines are chopped up into rectangular segments and don't look good zoomed in
  geom_line(data = . %>% filter(year==2021), aes(y = deaths, colour = deaths_type), size = 1.1) +
  scale_x_continuous(breaks = seq(0,60,by=12)) +
  ylim(0, 850) +  # start scale at 0 for better understanding the scale of deaths
  facet_grid(~place_of_death) +
  scale_colour_manual(values = c("red","blue"),  # custom legend for the two lines
                      labels = c("All deaths",
                                 "Non Covid-19 deaths"),
  ...
  geom_text(data = annotations_deaths_by_place_2020_and_2021 %>% filter(label==2021), aes(x=x, y=y, label=label), fontface = "bold") +
  geom_text(data = annotations_deaths_by_place_2020_and_2021 %>% filter(label==2020), aes(x=x, y=y, label=label), alpha = 0.6) +
  geom_text(data = arrows_deaths_by_place_2020_and_2021 %>% filter(geom=="text"), aes(x=x,y=y,label=label)) +
  geom_curve(data = arrows_deaths_by_place_2020_and_2021 %>% filter(geom=="curve") %>% slice(1), aes(x=x,y=y,xend=xend,yend=yend), curvature = 0.20, arrow = arrow(length = unit(0.2, "cm")), alpha = 0.5) +
  geom_curve(data = arrows_deaths_by_place_2020_and_2021 %>% filter(geom=="curve") %>% slice(2), aes(x=x,y=y,xend=xend,yend=yend), curvature = 0.20, arrow = arrow(length = unit(0.2, "cm")), linetype = "dashed") +
  theme(legend.position = "top", plot.caption = element_text(size = 10, colour = "gray60")) +
  labs(x = "Week number",
       y = "Weekly deaths (N)",
       subtitle = date_range_text,
...
save_output_file(filename = paste0(dir_outputs,"/figure_deaths_by_place_2020_and_2021"), extensions = c(".pdf",".svg",".png"), plot = figure_deaths_by_place_2020_and_2021, width = 28, height = 14, dpi = 300, units = "cm")
```
...
## Annual deaths by cause, 2020 only
...
## Deaths by cause, 2020 to now

It would be great to have a separate 2019 bar to show changes from last year only, however, cause of death data are only provided as an average 2015-2019!
...
# Proportion of cause of death by location
...
```{r}
(
figure_cause_of_death_by_location <-
  ...
  labs(
    ...
    # title = "More cancer deaths in 2020 and 2021 happened at <span style='color:#31688E;'>Home</span> than in <span style='color:#440154;'>Hospital</span>",
    title = "In 2020 and 2021 more cancer deaths happened at <span style='color:#31688E;'>Home</span> than in <span style='color:#440154;'>Hospital</span>",
    caption = paste0(c(source_nrs_text), collapse = "\n")
  ) +
  theme(
...
save_output_file(filename = paste0(dir_outputs,"/figure_cause_of_death_by_location"), extensions = c(".pdf",".png"), plot = figure_cause_of_death_by_location, width = 26, height = 16, units = "cm", dpi = 300)
```
...
# Covid as underlying vs contributory cause

```{r}
(figure_covid_contributory_vs_underlying_cause <-
annual_covid_contributory_vs_underlying_cause %>%
  pivot_longer(cols = -c(year, place_of_death, deaths_covid_related), names_to = "covid_category", values_to = "value") %>%
  extract(col = covid_category, into = c("measure", "covid_category"), regex = "(covid_19|proportion)_(.*)", remove = TRUE) %>%
  mutate(
    measure = if_else(measure == "covid_19", "n", measure),
    covid_category = str_remove(covid_category, pattern = "\\_.*$"),
    covid_category = paste0(covid_category, " cause"),
    year = str_replace(year, pattern = "2021", replacement = "2021*"),
    year = factor(year, levels = c("2020", "2021*", "2020-2021*")),
    place_of_death = factor(place_of_death, levels = c("Hospital", "Home & other non-institution", "Care home"))
  ) %>%
  filter(measure =="proportion") %>%
  filter(place_of_death != "Other") %>%
  ggplot(aes(x = place_of_death, y = value, group = covid_category, fill = covid_category)) +
    facet_grid(~year) +
    geom_col(position = position_stack()) +
    # geom_col(position = position_dodge(width = 1)) +
    scale_y_continuous(labels = function(x) scales::percent(x, accuracy = 1)) +
    scale_fill_viridis_d(labels = c("Contributory", "Underlying")) +
    # geom_text(data = . %>% filter(covid_category == "contributory cause"), inherit.aes = TRUE, aes(label = paste0("Contributory cause: ", scales::percent(value, accuracy = 0.1))), x = 0.7, nudge_y = -0.2, hjust = 1) +
    # geom_text(data = . %>% filter(covid_category == "underlying cause"), inherit.aes = TRUE, aes(label = paste0("Underlying cause: ", scales::percent(value, accuracy = 0.1))), x = 0.5, nudge_y = 0.2, hjust = 1) +
    geom_label(data = . %>% filter(covid_category == "contributory cause"), inherit.aes = TRUE, aes(label = scales::percent(value, accuracy = 0.1), y = 1.02-value), fill = "white",  hjust = 0.5) +
    geom_label(data = . %>% filter(covid_category == "underlying cause"), inherit.aes = TRUE, aes(label = scales::percent(value, accuracy = 0.1), y = value-0.2), fill = "white", hjust = 0.5) +
    labs(
      x = NULL, y = NULL,
      subtitle = paste0(c(paste0("* ",only_2021_text)),collapse="\n"),
      caption = paste0(c(source_nrs_text),collapse="\n")
      ) +
...
save_output_file(filename = paste0(dir_outputs,"/figure_covid_contributory_vs_underlying_cause"), extensions = c(".pdf",".svg",".png"), plot = figure_covid_contributory_vs_underlying_cause, width = 6, height = 6, units = "in", dpi = 300)
```
