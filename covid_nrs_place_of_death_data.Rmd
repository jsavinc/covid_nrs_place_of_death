...
```{r}
# TODO: change this to also keep track of latest available year
# TODO: modify above notes to include 2022 data also
# TODO: 2020 data aren't included in the latest files, so I might need to keep them separate
...
weekly_deaths_overall <-
  ...
  mutate(
    ...
    week_number_run_over = if_else(year == 2021, 53L + week_number, week_number)  # invent a "run-over" week number - weeks 54+ are in 2021, weeks 106+ are in 2022
  ) %>%
...
current_and_historical_deaths_by_cause_and_location <-
  bind_rows(
    ...
    total_deaths_cause_of_death %>%
      select(year, place_of_death, cause_of_death, deaths) %>%
      group_by(place_of_death,  cause_of_death) %>%
      summarise(deaths = sum(deaths), year = "2020-2021", .groups = "drop"),
    ## historical data
    total_deaths_cause_of_death %>%
...
table_annual_death_rates <-
  ...
  mutate(
    proportion_covid_deaths = scales::percent(deaths_covid_related / deaths_all_causes, accuracy = 0.1),
    change_deaths_all_causes_from_historical = scales::percent((deaths_all_causes / mean_annual_deaths_2015_2019) - 1, accuracy = 0.1),
    change_non_covid_deaths_from_historical = scales::percent((deaths_non_covid / mean_annual_deaths_2015_2019) - 1, accuracy = 0.1)
  )

table_annual_death_rates_by_cause <-
  total_deaths_cause_of_death %>%
  select(-matches("min|max")) %>%
  filter(year %in% c(2020:2021)) %>%
  mutate(
    change_from_historical_annual_deaths_2015_2019 = scales::percent(ratio_annual_deaths_to_historical_mean - 1, accuracy = 0.1) %>% if_else(. == "Inf", NA_character_, .)
    ) %>%
  mutate(temporary_group_for_all_causes = cause_of_death=="All") %>%
  group_by(year,place_of_death, temporary_group_for_all_causes) %>%
  mutate(
    proportion_annual_deaths_in_place = scales::percent(deaths / sum(deaths), accuracy = 0.1) %>% if_else(. == "100.0%", NA_character_, .),
    historical_proportion_annual_deaths_in_place = scales::percent(annual_deaths_mean / sum(annual_deaths_mean), accuracy = 0.1) %>% if_else(. == "100.0%", NA_character_, .)
    ) %>%
  ungroup %>%
  rename(mean_annual_deaths_2015_2019 = annual_deaths_mean) %>%
  select(-c(ratio_annual_deaths_to_historical_mean, temporary_group_for_all_causes))

notes_annual_death_rates <-
  tibble( notes = c(
    glue::glue("Note: these data are based on weekly figures reported by NRS; years don't align perfectly with calendar years, for example: week 1 of 2020 started on {ISOweek2date('2020-W01-1')}, and week 53 of 2020 ended on {ISOweek2date('2020-W53-7')}; week 1 of 2021 started on {ISOweek2date('2021-W01-1')}, and week 52 of 2021 ended on {ISOweek2date('2021-W52-7')}.")
    )
  )

write.xlsx(
    x = list(
      "2020 + 2021" = table_annual_death_rates,
      "Causes of death (COD)" = table_annual_death_rates_by_cause,
      "notes" = notes_annual_death_rates
      ), 
    file = file.path("./outputs", "annual_death_rates_and_causes_of_death_2020_2021.xlsx")
      )
```
