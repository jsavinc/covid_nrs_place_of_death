...
```{r}
# TODO: change this so it works for 2023 and onwards
...
weekly_deaths_cause_of_death <-
  bind_rows(
    ...
    weekly_deaths_cause_of_death_2020 %>% 
      select(-deaths_mean) %>%
      filter(cause_of_death != "All")  # don't need all cause data in causes breakdown!
  )
...
Cause of death data for 2015-2019 was only given in terms of weekly averages over 5 years. With the later historic periods (2016-2019 + 2021 and so on) I compute a weighted average between the 5-year average and the current weekly data.

The below compromise figure is no longer used, but the reasoning for the weighted average is the same (except for the quirk of the denominator of 6 in the below case, where I'm including the 5 years of historic data plus the one year of current data, hence 6 years in the denominator):
