---
title: "Place of death in NRS COVID-19 deaths: data processing for analysis, 2023 data"
author: "Jan Savinc"
...
weeks_available_2023 <- max(merged_deaths_overall$week_number[merged_deaths_overall$year==2023])
most_recent_date_available_2023 <- max(merged_deaths_overall$date_w_c)

earliest_date <- min(merged_deaths_overall$date_w_c)
current_year <- isoyear(most_recent_date_available_2023)

weeks_available_2023_disaggregated <- max(merged_deaths_age$week_number[merged_deaths_age$year==2023])
most_recent_date_available_2023_disaggregated <- max(merged_deaths_age$date_w_c)
```
...
table_current_deaths_compared_to_past <-
  bind_rows(
    ...
    merged_deaths_overall %>%
      ...
      summarise(
        period = glue::glue("Data from week 1, 2020 to week {weeks_available_2023}, 2022"),
    ...
    group_by(place_of_death) %>%
    summarise(
      period = glue::glue("2023, up to week {weeks_available_2023}"),
      historic_period = "2015-2019",
      deaths_all_causes = sum(deaths_all_causes),
      deaths_non_covid = sum(deaths_non_covid, na.rm = TRUE),
      deaths_covid_related = sum(deaths_covid_related, na.rm = TRUE),
